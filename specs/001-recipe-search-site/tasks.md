...
- [ ] T061 [US5] Write `build/CNAME` containing `recipes.nateemma.com` from `src/build.py`, so the custom domain survives every deploy (research.md R10)
- [ ] T062 [US5] Write `build/.nojekyll` from `src/build.py`, so Pages serves the output as-is
- [ ] T063 [US5] Write `.github/workflows/deploy.yml`: on push to the default branch — checkout, set up Python 3.12, install `requirements.txt`, run `python -m src.build --strict`, `actions/upload-pages-artifact`, `actions/deploy-pages`, with `pages: write` and `id-token: write` permissions and a `pages` concurrency group
- [ ] T064 [US5] Add a `pytest` step to `.github/workflows/deploy.yml` before the build, so a parser regression blocks the deploy as surely as a validation failure does
...
- [ ] T069 Update `--corpus` in `~/code/RecipeScanner/Makefile` to point at this repository's `recipes/`, and confirm `make vocab` still runs there (FR-041, SC-014)
- [ ] T070 Confirm `data/vocabulary.json` was **not** regenerated by this work, and record in `README.md` that regenerating it is RecipeScanner's decision (FR-042)
- [ ] T071 Read `build/parse-report.md` end to end and confirm no sentence became a heading and no genuine label was demoted to prose (Principle VI)
...
- [ ] T073 [P] Add a `Makefile` or `justfile` with `build`, `test`, `serve` and `migrate` targets so the common commands are discoverable
- [ ] T074 [P] Confirm the generated site contains no taxonomy browse or index pages and no Stork artifact (FR-032, SC-012)
- [ ] T075 Measure and record: full build time, `recipes.json` size, and search latency for the whole corpus, against SC-004 and SC-010
...
- [ ] T077 Run the full quickstart.md end to end as a final acceptance pass
